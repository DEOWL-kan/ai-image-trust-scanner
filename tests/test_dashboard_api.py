...
def _use_temp_report_store(monkeypatch: Any, tmp_path: Path) -> None:
...
def _save_report(filename: str, final_label: str, risk_level: str, confidence: float, batch_id: str | None = None) -> None:
    record = report_store.make_report_record(
        detection_data={
            ...
            "risk_level": risk_level,
            "confidence": confidence,
            "user_facing_summary": f"SQLite summary for {filename}",
            "recommendation": {"message": "Review when needed."},
        },
        source_type="batch" if batch_id else "single",
        batch_id=batch_id,
    )
    report_store.save_report(record)


def test_dashboard_summary_empty_history_data(monkeypatch: Any, tmp_path: Path) -> None:
    _use_temp_report_store(monkeypatch, tmp_path)
...
def test_dashboard_summary_prefers_sqlite_reports(monkeypatch: Any, tmp_path: Path) -> None:
    _use_temp_report_store(monkeypatch, tmp_path)
    _save_report("sqlite_ai.jpg", "ai", "high", 0.9)
    _save_report("sqlite_batch_real.jpg", "real", "low", 0.8, batch_id="batch_sqlite")

    payload = dashboard_summary.build_dashboard_payload(limit_recent=10, include_debug=True)
    summary = payload["summary"]

    assert payload["debug"]["data_source"] == "sqlite_reports"
    assert summary["total_detections"] == 2
    assert summary["single_detection_count"] == 1
    assert summary["batch_detection_count"] == 1
    assert summary["total_images_processed"] == 2
    assert summary["final_label_distribution"]["ai_generated"] == 1
    assert summary["final_label_distribution"]["real"] == 1
    assert all(isinstance(item["history_file"], str) for item in payload["recent_results"])
    assert all(isinstance(item["history_file"], str) for item in payload["recent_batches"])
