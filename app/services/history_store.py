...
HISTORY_DIR = Path(os.getenv("MINERVA_API_HISTORY_DIR", str(PROJECT_ROOT / ".tmp" / "api_history"))).expanduser()
...
def _safe_history_path(filename: str) -> Path:
    ...
    if not name or name != filename or Path(filename).suffix.lower() != ".json":
        raise InvalidHistoryFilenameError("Only JSON filenames inside the configured API history directory can be read.")
