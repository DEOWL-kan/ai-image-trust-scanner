...
def _compact_result_from_report(record: dict[str, Any]) -> dict[str, Any]:
    final_label = _normalize_final_label(record.get("final_label"))
    ...
    history_file = str(record.get("history_file") or "")
    return {
        "id": str(record.get("report_id") or record.get("id") or ""),
        "report_id": str(record.get("report_id") or record.get("id") or ""),
        "timestamp": str(record.get("created_at") or record.get("updated_at") or ""),
        "filename": str(Path(str(filename)).name or filename),
        "final_label": final_label,
        "risk_level": risk_level,
        "confidence": _safe_float(record.get("confidence")),
        "user_facing_summary": str(record.get("user_facing_summary") or record.get("report_summary") or ""),
        "recommendation": _recommendation_text(record.get("recommendation")),
        "history_type": str(record.get("source_type") or record.get("history_type") or "report"),
        "history_file": history_file,
        "batch_id": record.get("batch_id"),
    }


def load_dashboard_reports(limit_recent: int | None = None) -> dict[str, Any]:
    warnings: list[str] = []
    try:
        if limit_recent is not None:
            snapshot = report_store.dashboard_snapshot(limit_recent=limit_recent)
        ...
        return {
            ...
            "single_history_count": 0,
            "debug": {
                "history_source": str(report_store.REPORT_DB_PATH),
                "data_source": "sqlite_reports",
                "result_files_loaded": 0,
                "skipped_files": 0,
                "warnings": [f"sqlite_reports: {type(exc).__name__}: {exc}"],
                "schema_version": SCHEMA_VERSION,
            },
        }

    results = [_compact_result_from_report(record) for record in records]
    results.sort(key=lambda item: _parse_sort_datetime(item.get("timestamp")), reverse=True)
    ...
    if snapshot is not None:
        batches = [
            {
                ...
                "history_file": str(item.get("history_file") or ""),
            }
        ...
        batches = []
        for batch_id, items in batch_groups.items():
            latest = max(items, key=lambda item: _parse_sort_datetime(item.get("timestamp")))
            batches.append(
                {
                    "id": batch_id,
                    ...
                    "succeeded": len(items),
                    "failed": 0,
                    "history_file": str(latest.get("history_file") or ""),
                }
            )
    batches.sort(key=lambda item: _parse_sort_datetime(item.get("timestamp")), reverse=True)

    single_count = sum(1 for item in results if not item.get("batch_id"))
    return {
        "results": results,
        "batches": batches,
        ...
        "debug": {
            "history_source": str(report_store.REPORT_DB_PATH),
            "data_source": "sqlite_reports",
            "result_files_loaded": len(records),
            "skipped_files": 0,
            "warnings": warnings,
            "schema_version": SCHEMA_VERSION,
        },
        "aggregate": snapshot.get("aggregate") if snapshot is not None else None,
    }


def load_dashboard_records(history_dir: Path | None = None, limit_recent: int | None = None) -> dict[str, Any]:
    if history_dir is not None:
        loaded = load_dashboard_history(history_dir)
        loaded["debug"]["data_source"] = "history_json"
        return loaded

    reports = load_dashboard_reports(limit_recent=limit_recent)
    if reports["results"]:
        return reports

    loaded = load_dashboard_history(None)
    loaded["debug"]["data_source"] = "history_json_fallback"
    return loaded
...
def build_dashboard_payload(
    *,
    ...
    loaded = load_dashboard_records(history_dir, limit_recent=safe_limit if history_dir is None else None)
    results = loaded["results"]
    batches = loaded["batches"]
    aggregate = loaded.get("aggregate") if isinstance(loaded.get("aggregate"), dict) else None

    if aggregate:
        final_counts = _empty_counter(FINAL_LABELS)
        final_counts.update({str(key): int(value) for key, value in (aggregate.get("final_label_distribution") or {}).items()})
        risk_counts = _empty_counter(RISK_LEVELS)
        risk_counts.update({str(key): int(value) for key, value in (aggregate.get("risk_level_distribution") or {}).items()})
        confidence_counts = _empty_counter(CONFIDENCE_BUCKETS)
        confidence_counts.update({str(key): int(value) for key, value in (aggregate.get("confidence_distribution") or {}).items()})
        total_images = _safe_int(aggregate.get("total_images_processed"))
        average_confidence = _safe_float(aggregate.get("average_confidence"))
    ...
    chart_data = {
        ...
        "daily_trend": aggregate.get("daily_trend", []) if aggregate else _daily_trend(results),
    }
    ...
    payload = {
        ...
        "summary": {
            "total_detections": _safe_int(aggregate.get("total_detections")) if aggregate else loaded["single_history_count"] + len(batches),
            "single_detection_count": _safe_int(aggregate.get("single_detection_count")) if aggregate else loaded["single_history_count"],
            "batch_detection_count": _safe_int(aggregate.get("batch_detection_count")) if aggregate else len(batches),
...
def build_recent_results_payload(
    *,
    ...
    results = load_dashboard_records(history_dir)["results"]
    if normalized_label:
