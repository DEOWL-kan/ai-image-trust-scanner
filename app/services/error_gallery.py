...
DEFAULT_ERROR_GALLERY_DIR = PROJECT_ROOT / "examples" / "error_gallery"
DATA_ROOT = Path(os.getenv("MINERVA_ERROR_GALLERY_MEDIA_DIR", str(DEFAULT_ERROR_GALLERY_DIR / "media"))).expanduser()
DATA_ROOT_ABS = Path(os.path.normpath(str(DATA_ROOT)))
DAY23_DIR = Path(os.getenv("MINERVA_ERROR_GALLERY_DIR", str(DEFAULT_ERROR_GALLERY_DIR / "day23"))).expanduser()
...
REVIEW_NOTES_PATH = Path(
    os.getenv("MINERVA_ERROR_REVIEW_NOTES_PATH", str(PROJECT_ROOT / ".tmp" / "day24_error_review_notes.json"))
).expanduser()
...
def _is_under_data_root(path: Path) -> bool:
    try:
        path.relative_to(DATA_ROOT_ABS)
    except ValueError:
        return False
    return True
...
def _resolve_image_path(value: Any) -> Path | None:
    if value in (None, ""):
        return None
    path = _project_absolute(Path(str(value)))
...
def is_safe_data_image_path(path: Path) -> bool:
    candidate = _project_absolute(path)
    if not _is_under_data_root(candidate):
        return False
    return candidate.suffix.lower() in IMAGE_EXTENSIONS


def _image_url(path: Path | None) -> str | None:
    if not path or not is_safe_data_image_path(path):
        return None
    relative = _project_absolute(path).relative_to(DATA_ROOT_ABS).as_posix()
...
def _relative_source_folder(path: Path | None) -> str:
    if not path:
        return "unknown"
    try:
        return _project_absolute(path).parent.relative_to(DATA_ROOT_ABS).as_posix()
