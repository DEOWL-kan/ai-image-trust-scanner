...
def _normalize_record(
    *,
    ...
    record = {
        ...
        "provenance": _first_defined(result.get("provenance"), raw.get("provenance")),
        "detector_result_schema_version": _first_defined(result.get("detector_result_schema_version"), raw.get("detector_result_schema_version"), "legacy_unavailable"),
        "detector_results": _first_defined(result.get("detector_results"), raw.get("detector_results"), []),
        "detector_summary": _first_defined(
            result.get("detector_summary"),
            raw.get("detector_summary"),
            {
                "schema_version": "legacy_unavailable",
                "enabled_count": 0,
                "ok_count": 0,
                "error_count": 0,
                "skipped_count": 0,
        ...
        "detector_registry_version": _first_defined(result.get("detector_registry_version"), raw.get("detector_registry_version"), "legacy_unavailable"),
        "threshold_profile": _first_defined(result.get("threshold_profile"), raw.get("threshold_profile"), "legacy_unavailable"),
        "model_adapter_version": _first_defined(result.get("model_adapter_version"), raw.get("model_adapter_version"), "legacy_unavailable"),
...
def _legacy_to_persistent_record(record: dict[str, Any]) -> dict[str, Any]:
    data = {
        ...
        "detector_result_schema_version": record.get("detector_result_schema_version"),
        "detector_results": record.get("detector_results"),
        "detector_summary": record.get("detector_summary"),
        "detector_registry_version": record.get("detector_registry_version"),
        "threshold_profile": record.get("threshold_profile"),
        "model_adapter_version": record.get("model_adapter_version"),
...
def search_reports(
    *,
    ...
    if history_dir is None:
        bootstrap_sqlite_from_history()
        payload = report_store.search_report_summaries(
            q=q,
            risk_level=risk,
            final_label=label,
...
def review_queue(limit: int = 20, history_dir: Path | None = None) -> dict[str, Any]:
    if history_dir is None:
        bootstrap_sqlite_from_history()
        return public_report_payload(report_store.review_queue_summaries(limit=limit))
