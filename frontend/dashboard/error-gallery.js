const ERROR_ENDPOINTS = {
  get summary() { return apiUrl("/api/v1/errors/summary"); },
  get list() { return apiUrl("/api/v1/errors"); },
  detail: (id) => apiUrl(`/api/v1/errors/${encodeURIComponent(id)}`),
  review: (id) => apiUrl(`/api/v1/errors/${encodeURIComponent(id)}/review`),
};
...
function imageMarkup(item, className = "gallery-thumb") {
  ...
  return `
    <img
      class="${className}"
      src="${escapeHtml(apiUrl(item.image_url))}"
