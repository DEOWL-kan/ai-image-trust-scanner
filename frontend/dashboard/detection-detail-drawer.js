(function () {
  ...
  function apiUrl(path) {
    if (window.MinervaApi?.apiUrl) {
      return window.MinervaApi.apiUrl(path);
    }
    return path;
  }

  function apiUrlWithAuth(path) {
    if (window.MinervaApi?.apiUrlWithAuth) {
      return window.MinervaApi.apiUrlWithAuth(path);
    }
    return apiUrl(path);
  }

  function authHeaders() {
    return window.MinervaApi?.authHeaders?.() || {};
  }

  async function ensureApiBaseReachable() {
    if (window.MinervaApi?.ensureApiBaseReachable) {
      await window.MinervaApi.ensureApiBaseReachable({ timeoutMs: 5000 });
    }
  }
  ...
  Object.assign(ui.en, {
    ...
    openaiProvenance: "OpenAI provenance",
    generator: "Generator",
    issuer: "Issuer",
    validity: "Validity",
  });
  ...
  function normalizeDetectionRecord(record) {
    ...
    const provenance = firstDefined(
      result.provenance,
      raw.provenance,
      getValue(raw, "report_payload_json.api_data.provenance"),
      getValue(raw, "export_payload_json.provenance"),
      getValue(raw, "debug_evidence.provenance"),
    );
    return {
      id: String(id),
  ...
  function renderProvenance(value) {
    ...
    const hasData = Object.keys(provenance).length > 0;
    if (!hasData) {
      return `<p class="detail-muted">${escapeHtml(tr("noEvidence"))}</p>`;
    }
    const verified = provenance.verified || {
      c2pa_present: provenance.c2pa_present,
      c2pa_readable: provenance.c2pa_readable,
      c2pa_valid: provenance.c2pa_valid,
      c2pa_issuer: provenance.c2pa_issuer,
      c2pa_generator: provenance.c2pa_generator,
      openai_provenance_detected: provenance.openai_provenance_detected,
      confidence: provenance.provenance_confidence,
    };
    const markers = provenance.unverified_markers || {
      binary_c2pa_marker_found: provenance.binary_c2pa_marker_found,
      binary_openai_marker_found: provenance.binary_openai_marker_found,
      binary_gpt_image_marker_found: provenance.binary_gpt_image_marker_found,
      marker_confidence: provenance.binary_c2pa_marker_found || provenance.binary_openai_marker_found ? "weak" : "none",
      used_for_final_decision: false,
    };
    const diagnostics = provenance.diagnostics || { c2pa_probe_status: provenance.c2pa_probe_status };
    const markerFound = Boolean(markers.binary_c2pa_marker_found || markers.binary_openai_marker_found || markers.binary_gpt_image_marker_found);
    let evidenceTier = "Unable to verify";
    let c2paStatus = "Unavailable";
    let note = provenance.user_note || "";
    if (verified.openai_provenance_detected && verified.c2pa_readable) {
      evidenceTier = "Verified provenance";
      c2paStatus = "Verified OpenAI C2PA detected";
      note = "Verified OpenAI C2PA provenance detected. This is strong positive evidence of AI generation.";
    } else if (diagnostics.c2pa_probe_status === "claim_cbor_decode_error") {
      c2paStatus = "Unable to verify / claim decode error";
      note = "C2PA data may be present, damaged, unsupported, or unreadable by the local tool. Any binary markers are weak diagnostic evidence only.";
    } else if (markerFound) {
      evidenceTier = "Unverified provenance markers";
      c2paStatus = "OpenAI/C2PA binary markers found, but not verified";
      note = "Unverified C2PA/OpenAI binary markers were found, but no readable verified manifest was available.";
    } else if (diagnostics.c2pa_probe_status === "no_manifest") {
      c2paStatus = "No readable C2PA metadata found";
      note = "C2PA provenance was not found after transformation. This does not prove the image is authentic; the system falls back to visual/model-based detection.";
    }
    const openaiStatus = verified.openai_provenance_detected ? "Yes" : "No";
    return `
      <div class="detail-meta-grid">
        <div><span>Evidence tier</span><strong>${escapeHtml(evidenceTier)}</strong></div>
        <div><span>${escapeHtml(tr("c2paStatus"))}</span><strong>${escapeHtml(c2paStatus)}</strong></div>
        <div><span>${escapeHtml(tr("openaiProvenance"))}</span><strong>${escapeHtml(openaiStatus)}</strong></div>
        <div><span>${escapeHtml(tr("generator"))}</span><strong>${escapeHtml(verified.c2pa_generator || "N/A")}</strong></div>
        <div><span>${escapeHtml(tr("issuer"))}</span><strong>${escapeHtml(verified.c2pa_issuer || "N/A")}</strong></div>
        <div><span>${escapeHtml(tr("confidence"))}</span><strong>${escapeHtml(verified.confidence || "unknown")}</strong></div>
        <div><span>${escapeHtml(tr("validity"))}</span><strong>${escapeHtml(verified.c2pa_valid === null || verified.c2pa_valid === undefined ? "unknown" : String(verified.c2pa_valid))}</strong></div>
      </div>
      <p>${escapeHtml(note)}</p>
    `;
  }
...
<body>
  <main>
    ...
    <section><h2>Image</h2><div class="image">${record.image_url ? `<img src="${escapeHtml(apiUrl(record.image_url))}" alt="${escapeHtml(record.filename)}">` : "Image preview unavailable"}</div></section>
    ...
    <section><h2>C2PA Provenance</h2>${renderProvenance(record.provenance)}</section>
  ...
  function openHtmlPreview(record) {
    const normalized = normalizeDetectionRecord(record);
    if (normalized.id) {
      const opened = window.open(apiUrlWithAuth(`/api/v1/reports/${encodeURIComponent(normalized.id)}/html`), "_blank", "noopener");
  ...
  function renderDrawer(record, options = {}) {
    ...
    const image = normalized.image_url
      ? `<img class="detail-image" src="${escapeHtml(apiUrl(normalized.image_url))}" alt="${escapeHtml(normalized.filename)}">`
    ...
    drawer.innerHTML = `
      ...
      <section class="detail-section">
        ...
        ${renderProvenance(normalized.provenance)}
      </section>
      <section class="detail-section">
  ...
  async function saveReview(status) {
    ...
    const response = await fetch(apiUrl(`/api/v1/reports/${encodeURIComponent(recordId)}/review`), {
      method: "PATCH",
      headers: {
  ...
  async function hydrateHistory(summary, options) {
    ...
    try {
      await ensureApiBaseReachable();
      const history = await fetch(apiUrl(`/history/${encodeURIComponent(summary.history_file)}`), {
        cache: "no-store",
  ...
  async function hydrateReportDetail(summary, options) {
    ...
    try {
      await ensureApiBaseReachable();
      const detail = await fetch(apiUrl(`/api/v1/reports/${encodeURIComponent(recordId)}`), {
        cache: "no-store",
        headers: { Accept: "application/json", ...authHeaders() },
      }).then((response) => {
        if (!response.ok) throw new Error(`Detail fetch failed: ${response.status}`);
