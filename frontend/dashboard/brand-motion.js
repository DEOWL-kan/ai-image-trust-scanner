(function () {
  ...
  const palette = [
    { r: 0, g: 174, b: 209, a: 0.3 },
    { r: 53, g: 188, b: 155, a: 0.24 },
    { r: 200, g: 146, b: 46, a: 0.28 },
    { r: 77, g: 127, b: 217, a: 0.18 },
    { r: 255, g: 255, b: 255, a: 0.22 },
  ];
  ...
  function countParticles() {
    if (reducedMotion) return width < 700 ? 8 : 14;
    if (width < 700) return 26;
    if (width < 1180) return 46;
    return 68;
  }
  ...
  function drawConnections(points) {
    ...
    context.save();
    context.strokeStyle = "rgba(0, 143, 173, 0.16)";
    context.lineWidth = 1;
  ...
  function detectPointerShape(target) {
    ...
    if (element.closest(".hero-section, .website-hero")) return "natural";
  ...
  function animateTrustLine(panel, force = false) {
    ...
    const metrics = Array.from(panel.querySelectorAll("[data-hero-metric]"));
    const trustMetric = panel.querySelector('[data-hero-metric="backend"]');
    const parsedTarget = Number.parseInt(String(trustMetric?.dataset.target || trustMetric?.textContent || ""), 10);
    const trustTarget = Number.isFinite(parsedTarget) ? Math.max(0, Math.min(100, parsedTarget)) : 0;
    if (!metrics.length || !line || reducedMotion) {
      if (line) line.style.width = `${trustTarget}%`;
      return;
    }
    panel.classList.remove("is-telemetry-live");
    void panel.offsetWidth;
    panel.classList.add("is-telemetry-live");
    line.style.width = "0%";
    metrics.forEach((metric) => {
      metric.dataset.finalValue = metric.textContent.trim() || metric.dataset.finalValue || "";
      metric.classList.remove("is-counting");
      metric.textContent = metric.dataset.finalValue && /\d/.test(metric.dataset.finalValue) ? "0" : metric.dataset.finalValue;
    });
    ...
    function step(time) {
      ...
      line.style.width = `${trustTarget * eased}%`;
      metrics.forEach((metric, index) => {
        const finalValue = metric.dataset.finalValue || "";
        const match = finalValue.match(/^(\d+)(.*)$/);
        if (!match) {
          if (progress > Math.min(0.28 + index * 0.08, 0.72)) metric.textContent = finalValue;
          return;
        }
        metric.classList.add("is-counting");
        const target = Number.parseInt(match[1], 10);
        metric.textContent = `${Math.round(target * eased)}${match[2] || ""}`;
      });
      if (progress < 1) window.requestAnimationFrame(step);
      else {
        metrics.forEach((metric) => {
          metric.textContent = metric.dataset.finalValue || metric.textContent;
          metric.classList.remove("is-counting");
        });
      }
    }
  ...
  function updateNavState() {
    ...
    const watermark = document.querySelector(".hero-ambient-stage");
    if (watermark && !reducedMotion && window.innerWidth >= 900) {
      const x = pointer.active ? (pointer.x / Math.max(width, 1) - 0.5) * 8 : 0;
      const y = pointer.active ? (pointer.y / Math.max(height, 1) - 0.5) * 6 : 0;
      const scroll = Math.max(Math.min(current * -0.018, 0), -10);
      watermark.style.transform = `translate(${x}px, ${y + scroll}px)`;
    }
  }
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            if (motion === "hero") sectionShape = "natural";
            if (motion === "scanner") sectionShape = "scanner";
            if (motion === "chain") sectionShape = "chain";
            if (motion === "roadmap") sectionShape = "timeline";
          } else {
            section.classList.remove("is-motion-active", "motion-replay");
            if (section.dataset.motion === sectionShape) {
              sectionShape = "natural";
            }
          }
        });
      },
